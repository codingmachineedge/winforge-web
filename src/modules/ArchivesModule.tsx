...
// One advanced operation. `args` uses {archive} {src} {outdir} placeholders that
// are substituted with the quoted paths at run time — identical to ArchiveService.
interface Op {
  id: string;
  enTitle: string;
  zhTitle: string;
...
function op(
  ...
  zhBtn: string,
  args: string,
  o: { needsArchive?: boolean; needsSource?: boolean; destructive?: boolean; rar?: boolean; keywords: string },
): Op {
  return {
    id,
    enTitle,
...
const OPERATIONS: Op[] = [
  ...
  op('arc.inspect.test-password', 'Test (with password)', '測試（連密碼）', 'Test an encrypted archive using the placeholder password.', '用佔位密碼測試一個加密咗嘅壓縮檔。', 'Test', '測試', 't -p{password} {archive}', { keywords: 'test password encrypted 密碼 加密' }),
  ...
  op('arc.test-then-extract', 'Test archive first', '先測試', 'Test archive integrity before you extract with full paths.', '解壓之前先測試壓縮檔完唔完整。', 'Test', '測試', 't {archive} -y', { keywords: 'extract test verify 測試 解壓' }),
  op('arc.extract-stdout-note', 'Extract to stdout', '解壓去 stdout', 'Write the extracted bytes to standard output instead of a file.', '將解壓出嚟嘅內容寫去標準輸出，唔寫成檔案。', 'Extract', '解壓', 'x {archive} -so -y', { keywords: 'extract stdout so pipe 輸出 解壓' }),
  ...
  op('arc.mod-delete', 'Delete entry', '刪除項目', 'Delete the matching entries from inside the archive.', '將壓縮檔入面符合嘅項目刪除咗佢。', 'Delete', '刪除', 'd {archive} *.tmp', { destructive: true, keywords: 'delete remove 刪除 項目' }),
...
// split on whitespace but keep {archive}/{src}/{outdir}/{password} placeholders
// intact so paths with spaces stay as single argv items (runCommand handles
// quoting per-arg, so we must NOT pre-quote).
function buildArgv(
  template: string,
  ctx: { archive: string; src: string; outdir: string; password: string },
): string[] {
...
export function ArchivesModule() {
  ...
  async function run(o: Op) {
    ...
    if (o.rar) {
      setStatus({
        ok: false,
        text: pick(
          'RAR repair needs unrar.exe (WinRAR). 7-Zip cannot repair RAR archives.',
          'RAR 修復要 unrar.exe（WinRAR）。7-Zip 修唔到 RAR。',
        ),
      });
      // Still attempt via unrar on PATH.
    }
    setBusy(o.id);
    setStatus(null);
    ...
    try {
      ...
      const outdir = outDirFor(archive.trim());
      const argv = buildArgv(o.args, {
  ...
  const quick: { label: string; op: Op }[] = [
    { label: pick('List', '列出'), op: OPERATIONS[0]! },
    { label: pick('Test', '測試'), op: OPERATIONS[3]! },
    { label: pick('Extract here', '解壓到旁邊'), op: OPERATIONS[18]! },
    { label: pick('Benchmark', '效能測試'), op: OPERATIONS.find((o) => o.id === 'arc.benchmark')! },
  ];
  ...
  return (
    <div className="mod">
      ...
      {/* ---- Selection: archive + source paths ---- */}

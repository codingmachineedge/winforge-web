...
// custom-layouts.json), toggles the module and every behaviour flag by rewriting those JSON files
// (preserving the rest) and restarting the PowerToys runner, and launches PowerToys / the zone
// editor / the FancyZones settings page. Read-mostly and defensive; writes are explicit and gated.
...
$fzPath = Join-Path $root 'FancyZones\\settings.json'
$clPath = Join-Path $root 'FancyZones\\custom-layouts.json'
...
if (Test-Path -LiteralPath $clPath) {
  try {
    ...
    foreach ($it in $arr) { if ($it.name) { $custom += [string]$it.name } }
  } catch {}
}
[pscustomobject]@{
  installed = $installed; hostPath = $host2; version = $ver; running = $running; editorRunning = $editorRunning;
  moduleEnabled = $moduleEnabled; settingsExists = $settingsExists; customLayouts = @($custom); props = $props
}`;
}
...
export function FancyZonesModule() {
  ...
  // Launch PowerToys, open the zone editor, or open the FancyZones settings page.
  const runAction = async (kind: 'launch' | 'editor' | 'settings') => {
    ...
    try {
      ...
      if (kind === 'launch') {
        ...
        // Zone editor: prefer the editor exe next to PowerToys.exe, else signal the runner via the host.
        script =
          `$dir = Split-Path -Parent '${psq(host)}'; ` +
          `$ed = Join-Path $dir 'PowerToys.FancyZonesEditor.exe'; ` +
          `if (Test-Path -LiteralPath $ed) { Start-Process -FilePath $ed -WorkingDirectory $dir } ` +
          `else { Start-Process -FilePath '${psq(host)}' }; 'OK'`;
      }
  ...
  return (
    <div className="mod">
      ...
      <AsyncState loading={probe.loading} error={probe.error}>
        ...
        {/* ── Behaviour toggles (write FancyZones settings.json) ── */}
        <div className="panel">
          <div className="dt-wrap">

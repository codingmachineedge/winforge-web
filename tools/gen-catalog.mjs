...
// ---- Windows-tweak catalog (the desktop TweakCatalog: ~895 tweaks / 22 categories) ----------
// src/data/tweaks.ts is generated by tools/gen-tweaks.mjs from WinForge's own `--export-docs`.
// We surface each tweak category as a nav entry under "Windows 11 › All Tweaks", tagged
// module.tweaks.<catId> and rendered by TweaksBrowser. Registry writes are desktop-only, so
// native:false lets the browsable, searchable, bilingual catalog render in the web preview too.
// These entries ARE routable and searchable (they live in allModules) but are NOT counted as
// modules — moduleCount excludes them so the "314 modules" invariant holds; they are the 895
// tweaks' 22 category buckets. Each category packs its tweak titles into keywords so a global
// search for any tweak name (e.g. "bitlocker") surfaces its category.
const TWEAKS_TS = path.join(__dirname, '..', 'src', 'data', 'tweaks.ts');
function extractGenArray(src, marker) {
  const a = src.indexOf(marker);
  if (a < 0) return [];
  // Search for the array's opening bracket AFTER the marker text — the marker itself
  // contains "[]" (e.g. "TweakData[] ="), so we must skip past it.
  const start = src.indexOf('[', a + marker.length);
  const end = src.indexOf('\n];', start);
  if (start < 0 || end < 0) return [];
  return JSON.parse(src.slice(start, end + 2));
}
let tweakMods = [];
if (fs.existsSync(TWEAKS_TS)) {
  const twSrc = fs.readFileSync(TWEAKS_TS, 'utf8');
  const TW = extractGenArray(twSrc, 'export const tweaks: TweakData[] =');
  const TC = extractGenArray(twSrc, 'export const tweakCategories: TweakCategory[] =');
  const titlesByCat = {};
  for (const t of TW) (titlesByCat[t.cat] ||= []).push(t.en, t.zh);
  tweakMods = TC.map((c) => ({
    tag: `module.tweaks.${c.id}`,
    en: c.en,
    zh: c.zh,
    glyph: '',
    keywords: [c.en, c.zh, 'tweak', 'tweaks', '調校', 'windows', ...(titlesByCat[c.id] || [])].join(' '),
    native: false,
  }));
  const win11 = sections.find((s) => s.id === 'windows-11');
  const grp = win11 && win11.groups.find((g) => g.id === 'all-tweaks');
  if (grp) {
    grp.modules = tweakMods;
    grp.subgroups = [];
  } else {
    console.warn('gen-catalog: "windows-11 › all-tweaks" group not found; tweak categories not injected.');
  }
} else {
  console.warn('gen-catalog: src/data/tweaks.ts not found — run `node tools/gen-tweaks.mjs` first.');
}
...
// moduleCount is the real-module count (the desktop's ModuleRegistry — 314). The injected
// module.tweaks.* entries are the 895 tweaks' category buckets, not modules, so they are
// excluded here while remaining routable/searchable inside allModules.
const realModuleCount = allModules.filter((m) => !m.tag.startsWith('module.tweaks.')).length;
...
// ${realModuleCount} modules + ${tweakMods.length} tweak categories across ${sections.length} sections.
/* eslint-disable */
...
const body =
  ...
  `\nexport const moduleCount = ${realModuleCount};\n` +
  `\n/** Category buckets for the Windows-tweak catalog (module.tweaks.* nav entries). */\n` +
  `export const tweakCategoryCount = ${tweakMods.length};\n`;

fs.writeFileSync(OUT, header + body);
console.log(
  `Wrote ${OUT}: ${realModuleCount} modules + ${tweakMods.length} tweak categories, ${sections.length} sections`,
);
